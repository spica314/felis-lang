...
fn reports_lexer_error_patterns() {
    ...
    assert_parse_error("\"", "unterminated string literal", Span { start: 0, end: 1 });
    assert_parse_error(
        "\"\\u12",
    ...
    assert_parse_error("t", "unexpected end of input after `t`", Span { start: 0, end: 1 });
    assert_parse_error("tre", "unexpected token `tre`", Span { start: 0, end: 3 });
    assert_parse_error("-", "expected digit after `-`", Span { start: 0, end: 1 });
    assert_parse_error("1.", "expected digit after decimal point", Span { start: 0, end: 2 });
    assert_parse_error("1e+", "expected digit in exponent", Span { start: 0, end: 3 });
    assert_parse_error(
        r#""\uD800""#,
