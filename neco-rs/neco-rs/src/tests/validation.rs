...
#[test]
fn lowers_recursive_pure_function_over_runtime_type_rc_value() {
    let root = repo_root().join("tests/testcases/type-rc-parser-basic");
    let package = selected_fixture_package(&root, "type-rc-parser-basic");

    lower_package_to_program(&package).expect("lowering recursive pure call");
}
