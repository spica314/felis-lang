...
fn infers_simple_arrow_and_body() {
    ...
    let arrow_id = match def.type_.as_ref() {
        SynTerm::ArrowNodep(arrow) => arrow.ext.term_id.clone(),
        other => panic!("expected arrow type, got {other:?}"),
    };
    let body_id = match def.body.as_ref() {
        SynTerm::Unit(unit) => unit.ext.term_id.clone(),
    ...
    let expected_arrow = Term::Arrow {
        param: Box::new(Term::Unit),
        param_name: None,
        result: Box::new(Term::Unit),
    };
    assert_eq!(result.resolved_type(&arrow_id), Some(expected_arrow));
    assert_eq!(result.resolved_type(&body_id), Some(Term::Unit));
}
...
fn infers_dependent_arrow_parameter() {
    ...
    let (arrow_id, param_id) = match def.type_.as_ref() {
        SynTerm::ArrowDep(arrow) => (arrow.ext.term_id.clone(), arrow.ext.param_id.clone()),
    ...
    if let SynTerm::Variable(var) = def.body.as_mut() {
        ...
        *def.body = SynTerm::Variable(updated);
    }
...
"#,
    ...
    if let SynTerm::Variable(var) = def.type_.as_mut() {
        ...
        *def.type_ = SynTerm::Variable(updated);
    }

    let builtins = BuiltinTypes::new([("u64", Term::Integer(IntegerType::U64))]);
    ...
    assert_eq!(
        ...
        Some(Term::Integer(IntegerType::U64))
    );
    assert_eq!(
...
fn infers_integer_literal_default_to_u64() {
    ...
    assert_eq!(ty, Term::Integer(IntegerType::U64));
}
...
fn infers_integer_literal_suffixes() {
    ...
    assert_eq!(i32_ty, Term::Integer(IntegerType::I32));
    assert_eq!(u8_ty, Term::Integer(IntegerType::U8));
}

#[test]
fn infers_float_literal() {
    let f32_ty = literal_body_type("#definition main: () -> () { 1.0f32 }");
    assert_eq!(f32_ty, Term::F32);
}
