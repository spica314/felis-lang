use crate::{
    Parse, ParseError, Phase, PhaseParse, ProcTerm, StatementAssign, StatementBreak, StatementLet,
    StatementLetMut, StatementLoop, StatementMethodChainAssign, StatementReturn, token::Token,
};
...
pub enum Statement<P: Phase> {
    ...
    FieldAssign(StatementMethodChainAssign<P>),
...
impl Parse for Statement<PhaseParse> {
    fn parse(tokens: &[Token], i: &mut usize) -> Result<Option<Self>, ParseError> {
        ...
        if let Some(statement_field_assign) = StatementMethodChainAssign::parse(tokens, i)? {
