...
#fn main : () #with IO {
    ...
    #let _ : () #with IO <- IO::write stdout message 6i32;
    ()
}
```
...
```felis
#let stdin : FileDescriptor #with IO <- IO::stdin;
#let stdout : FileDescriptor #with IO <- IO::stdout;
#let path : PathBuf #with IO <- IO::pathbuf_new 32i32;
#letref #excl path_ref : &^ PathBuf #borrow path;
#let _ : () #with IO <- IO::pathbuf_push path_ref "message.txt";
#let fd : FileDescriptor #with IO <- IO::open path_ref 0i32 0i32;
```
...
```felis
#let bytes_ref : Array u8 1000i32 #with IO <- IO::array_new u8 1000i32;
#let len : i32 #with IO <- IO::read stdin bytes_ref 1000i32;
#let _ : () #with IO <- IO::write stdout bytes_ref len;
```
...
#let _ : () #with IO <- IO::pathbuf_push path_ref "message.txt";
#let fd : FileDescriptor #with IO <- IO::open path_ref 0i32 0i32;
#let bytes_arrayvl : ArrayVL u8 #with IO <- IO::arrayvl_new u8 128i32;
#letref #excl bytes_arrayvl_ref : &^ ArrayVL u8 #borrow bytes_arrayvl;
#let len : i32 #with IO <- IO::read fd bytes_arrayvl_ref 128i32;
#let _ : () #with IO <- IO::close fd;
```
...
#let path : PathBuf #with IO <- IO::pathbuf_new 32i32;
#letref #excl path_ref : &^ PathBuf #borrow path;
#let _ : () #with IO <- IO::pathbuf_push path_ref "created.txt";
#let fd : FileDescriptor #with IO <- IO::open path_ref 577i32 420i32;
#let message : & ArrayVL u8 = "open/write/close fixture\n";
#let _ : () #with IO <- IO::write fd message 25i32;
#let _ : () #with IO <- IO::close fd;
```
...
```felis
#let path_ref : & ArrayVL u8 #with IO <- IO::arg 1i32;
#let digits_ref : & ArrayVL u8 #with IO <- IO::arg 2i32;
```
